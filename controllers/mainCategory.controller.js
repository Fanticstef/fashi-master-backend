...
async function createRecord(req, res) {
    try {
        ...
        await data.save()
        res.status(200).send({ result: 'Done', data, message: 'Maincategory Created Successfully!' })
    } catch (error) {
...
async function updateRecord(req, res) {
    try {
        ...
        if (data) {
            ...
            await data.save()
            res.status(200).send({ result: "Done", data, message: 'Maincategory Update Successfully!' })
        }
        else
